...
class DistanceMatrixMDVRP:
    ...
    def setup_problem_parameters(self, depot_allocation=None):
        """Setup MDVRP parameters with realistic constraints"""
        print(f"\n{'='*60}")
        print("SETTING UP PROBLEM PARAMETERS")
        print("="*60)
        
        # **FIX 1: Use only major depots to reduce complexity**
        if depot_allocation is None:
            # Select only the most strategically located depots
            total_students = self.stops_df['num_students'].sum()
            buses_needed = int(np.ceil(total_students / 50))  # More conservative capacity
            
            # **FIX 2: Limit to 8-10 major depots instead of 24**
            num_major_depots = min(10, len(self.depot_indices))
            major_depot_indices = self.depot_indices[:num_major_depots]
            
            # Distribute buses among major depots
            base_buses = buses_needed // num_major_depots
            ...
            for i, depot_idx in enumerate(major_depot_indices):
                ...
                self.depot_bus_allocation[depot_idx] = max(3, buses)  # Minimum 3 buses per depot
                
            print(f"Using {num_major_depots} major depots (reduced from {len(self.depot_indices)}):")
        else:
        ...
        # **FIX 3: Increase capacity and reduce vehicles**
        self.vehicle_capacity = 50  # Slightly lower for more flexibility
    ...
    def solve_mdvrp(self, time_limit_seconds=1200):  # **FIX 4: Increase time limit**
        ...
        # **FIX 5: Add capacity constraint with slack**
        ...
        routing.AddDimensionWithVehicleCapacity(
            ...
            5,  # **SLACK: Allow 5 students over capacity if needed**
            data['vehicle_capacities'],
            True,
            'Capacity'
        )
        
        # **FIX 6: More aggressive search parameters**
        search_parameters = pywrapcp.DefaultRoutingSearchParameters()
        search_parameters.first_solution_strategy = (
            routing_enums_pb2.FirstSolutionStrategy.AUTOMATIC  # Let OR-Tools choose
        )
        search_parameters.local_search_metaheuristic = (
            routing_enums_pb2.LocalSearchMetaheuristic.AUTOMATIC  # Let OR-Tools choose
        )
        search_parameters.time_limit.FromSeconds(time_limit_seconds)
        search_parameters.log_search = True  # **Show progress**
        
        print(f"Starting optimization with aggressive search...")
        print(f"This may take up to {time_limit_seconds/60:.1f} minutes...")
        ...
        if solution:
            ...
            # **FIX 7: Provide debugging info**
            print("\nDebugging info:")
            print(f"- Problem might be too constrained")
            print(f"- Try reducing number of vehicles or increasing capacity")
            print(f"- Check if all locations are reachable")
            return None
...
def main():
    ...
    try:
        ...
        if solution:
            ...
            print("1. Reduce number of depots")
            print("2. Increase vehicle capacity")
            print("3. Add more vehicles")
            print("4. Increase time limit")
